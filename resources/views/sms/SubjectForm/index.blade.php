...
<tbody>
    @foreach($subjectform as $key=>$subjectforms)
   <tr>
       <td>{{ $key + 1 }}</td>
       <td>{{ $subjectforms->idno }}</td>
       <td>{{ $subjectforms->subject_name }}</td>
       <td>{{ $subjectforms->subject_type }}</td>
       <td>{{ $subjectforms->class }}</td>
       <td>{{ $subjectforms->created_at }}</td>
       <td>
            <a href="{{ route('SubjectForm.edit', $subjectforms->id) }}" class="btn btn-info btn-sm"><i class="material-icons">mode_edit</i></a> 
        <form id="delete-form-{{ $subjectforms->id }}" action="{{ route('SubjectForm.destroy', $subjectforms->id) }}" style="display: none;" method="post">
          @csrf
          @method('delete')
        </form>
        ...
        if(confirm('Are you sure? You want to delete this?')){
           ...
           document.getElementById('delete-form-{{ $subjectforms->id }}').submit();
          }else {
           event.preventDefault();
          }
