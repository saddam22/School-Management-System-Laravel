...
<tbody>
    @foreach($classroutineform as $key=>$classroutineform)
   <tr>
       <td>{{ $key + 1 }}</td>
       <td>{{ $classroutineform->day }}</td>
       <td>{{ $classroutineform->class }}</td>
       <td>{{ $classroutineform->subject_name }}</td>
       <td>{{ $classroutineform->section }}</td>
       <td>{{ $classroutineform->teacher_name }}</td>
       <td>{{ $classroutineform->time }}</td>
       <td>{{ $classroutineform->created_at }}</td>
       <td>
            <a href="{{ route('ClassRoutineForm.edit', $classroutineform->id) }}" class="btn btn-info btn-sm"><i class="material-icons">mode_edit</i></a> 
        <form id="delete-form-{{ $classroutineform->id }}" action="{{ route('ClassRoutineForm.destroy', $classroutineform->id) }}" style="display: none;" method="post">
          @csrf
          @method('delete')
        </form>
        ...
        if(confirm('Are you sure? You want to delete this?')){
           ...
           document.getElementById('delete-form-{{ $classroutineform->id }}').submit();
          }else {
           event.preventDefault();
          }
