...
<tbody>
    @foreach($noticeform as $key=>$noticeforms)
   <tr>
       <td>{{ $key + 1 }}</td>
       <td>{{ $noticeforms->title }}</td>
       <td>{{ $noticeforms->details }}</td>
       <td>{{ $noticeforms->posted }}</td>
       <td>{{ $noticeforms->date }}</td>
       <td>
            <a href="{{ route('NoticeForm.edit', $noticeforms->id) }}" class="btn btn-info btn-sm"><i class="material-icons">mode_edit</i></a> 
        <form id="delete-form-{{ $noticeforms->id }}" action="{{ route('NoticeForm.destroy', $noticeforms->id) }}" style="display: none;" method="post">
          @csrf
          @method('delete')
        </form>
        ...
        if(confirm('Are you sure? You want to delete this?')){
           ...
           document.getElementById('delete-form-{{ $noticeforms->id }}').submit();
          }else {
           event.preventDefault();
          }
