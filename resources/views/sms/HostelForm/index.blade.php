...
<tbody>            
    @foreach($hostelform as $key=>$hostelforms)
   <tr>
       <td>{{ $key + 1 }}</td>       
       <td>{{ $hostelforms->hostel_name }}</td>
       <td>{{ $hostelforms->room_number }}</td>
       <td>{{ $hostelforms->room_type }}</td>
       <td>{{ $hostelforms->number_bed }}</td>
       <td>{{ $hostelforms->cost_bed }}</td>
       <td>
            <a href="{{ route('HostelForm.edit', $hostelforms->id) }}" class="btn btn-info btn-sm"><i class="material-icons">mode_edit</i></a> 
        <form id="delete-form-{{ $hostelforms->id }}" action="{{ route('HostelForm.destroy', $hostelforms->id) }}" style="display: none;" method="post">
          @csrf
          @method('delete')
        </form>
        ...
        if(confirm('Are you sure? You want to delete this?')){
           ...
           document.getElementById('delete-form-{{ $hostelforms->id }}').submit();
          }else {
           event.preventDefault();
          }
