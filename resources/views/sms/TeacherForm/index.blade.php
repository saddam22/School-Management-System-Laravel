...
<tbody>
    @foreach($teacherform as $key=>$teacherforms)
   <tr>
       <td>{{ $key + 1 }}</td>
       <td>{{ $teacherforms->idno }}</td>
       <td><img src="{{ asset('uploads/teacherimages/'.$teacherforms->photo) }}" class="img-circle img-rounded" width="60" alt="no pic"></td>
       <td>{{ $teacherforms->fullname }}</td>
       <td>{{ $teacherforms->gender }}</td>
       <td>{{ $teacherforms->class }}</td>
       <td>{{ $teacherforms->subject }}</td>
       <td>{{ $teacherforms->section }}</td>
       <td>{{ $teacherforms->address }}</td>
       <td>{{ $teacherforms->phone }}</td>
       <td>{{ $teacherforms->email }}</td>
       <td>
            <a href="{{ route('TeacherForm.edit', $teacherforms->id) }}" class="btn btn-info btn-sm"><i class="material-icons">mode_edit</i></a> 
        <form id="delete-form-{{ $teacherforms->id }}" action="{{ route('TeacherForm.destroy', $teacherforms->id) }}" style="display: none;" method="post">
          @csrf
          @method('delete')
        </form>
        ...
        if(confirm('Are you sure? You want to delete this?')){
           ...
           document.getElementById('delete-form-{{ $teacherforms->id }}').submit();
          }else {
           event.preventDefault();
          }
