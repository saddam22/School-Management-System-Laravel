...
<tbody>
    @foreach($admissionform as $key=>$admissionforms)
   <tr>
       <td>{{ $key + 1 }}</td>
       <td>{{ $admissionforms->roll }}</td>
       <td><img src="{{ asset('uploads/admissionimages/'.$admissionforms->photo) }}" class="img-circle img-rounded" width="60" alt="no pic"></td>
       <td>{{ $admissionforms->fullname }}</td>
       <td>{{ $admissionforms->gender }}</td>
       <td>{{ $admissionforms->class }}</td>
       <td>{{ $admissionforms->section }}</td>
       <td>{{ $admissionforms->parents }}</td>
       <td>{{ $admissionforms->addresses }}</td>
       <td>{{ $admissionforms->dob }}</td>
       <td>{{ $admissionforms->phone }}</td>
       <td>{{ $admissionforms->email }}</td>
       <td>
            <a href="{{ route('AdmissionForm.edit', $admissionforms->id) }}" class="btn btn-info btn-sm"><i class="material-icons">mode_edit</i></a> 
        <form id="delete-form-{{ $admissionforms->id }}" action="{{ route('AdmissionForm.destroy', $admissionforms->id) }}" style="display: none;" method="post">
          @csrf
          @method('delete')
        </form>
        ...
        if(confirm('Are you sure? You want to delete this?')){
           ...
           document.getElementById('delete-form-{{ $admissionforms->id }}').submit();
          }else {
           event.preventDefault();
          }
