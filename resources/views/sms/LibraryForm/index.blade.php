...
<tbody>
    @foreach($libraryform as $key=>$libraryforms)
   <tr>
       <td>{{ $key + 1 }}</td>
       <td>{{ $libraryforms->idno }}</td>
       <td>{{ $libraryforms->book_name }}</td>
       <td>{{ $libraryforms->subject }}</td>
       <td>{{ $libraryforms->writter_name }}</td>
       <td>{{ $libraryforms->class }}</td>
       <td>{{ $libraryforms->publishing_date }}</td>
       <td>{{ $libraryforms->upload_date }}</td>
       <td>
            <a href="{{ route('LibraryForm.edit', $libraryforms->id) }}" class="btn btn-info btn-sm"><i class="material-icons">mode_edit</i></a> 
        <form id="delete-form-{{ $libraryforms->id }}" action="{{ route('LibraryForm.destroy', $libraryforms->id) }}" style="display: none;" method="post">
          @csrf
          @method('delete')
        </form>
        ...
        if(confirm('Are you sure? You want to delete this?')){
           ...
           document.getElementById('delete-form-{{ $libraryforms->id }}').submit();
          }else {
           event.preventDefault();
          }
