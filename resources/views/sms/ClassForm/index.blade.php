...
<tbody>
    @foreach($classform as $key=>$classforms)
   <tr>
       <td>{{ $key + 1 }}</td>
       <td>{{ $classforms->idno }}</td>
       <td>{{ $classforms->photo }}</td>
       <td>{{ $classforms->fullname }}</td>
       <td>{{ $classforms->gender }}</td>
       <td>{{ $classforms->class }}</td>
       <td>{{ $classforms->section }}</td>
       <td>{{ $classforms->date }}</td>
       <td>{{ $classforms->time }}</td>
       <td>{{ $classforms->phone }}</td>
       <td>{{ $classforms->email }}</td>
       <td>
            <a href="{{ route('ClassForm.edit', $classforms->id) }}" class="btn btn-info btn-sm"><i class="material-icons">mode_edit</i></a> 
        <form id="delete-form-{{ $classforms->id }}" action="{{ route('ClassForm.destroy', $classforms->id) }}" style="display: none;" method="post">
          @csrf
          @method('delete')
        </form>
        ...
        if(confirm('Are you sure? You want to delete this?')){
           ...
           document.getElementById('delete-form-{{ $classforms->id }}').submit();
          }else {
           event.preventDefault();
          }
