...
<tbody>
    @foreach($accountform as $key=>$accountforms)
   <tr>
       <td>{{ $key + 1 }}</td>
       <td>{{ $accountforms->fname }}</td>
       <td>{{ $accountforms->lname }}</td>
       <td>{{ $accountforms->user_type }}</td>
       <td>{{ $accountforms->gender }}</td>
       <td>{{ $accountforms->faname }}</td>
       <td>{{ $accountforms->moname }}</td>
       <td>{{ $accountforms->dob }}</td>
       <td>{{ $accountforms->religion }}</td>
       <td>{{ $accountforms->joining }}</td>
       <td>{{ $accountforms->email }}</td>
       <td>{{ $accountforms->subject }}</td>
       <td>{{ $accountforms->class }}</td>
       <td>{{ $accountforms->section }}</td>
       <td>{{ $accountforms->idno }}</td>
       <td>{{ $accountforms->phone }}</td>
       <td>{{ $accountforms->address }}</td>
       <td>
            <a href="{{ route('AccountForm.edit', $accountforms->id) }}" class="btn btn-info btn-sm"><i class="material-icons">mode_edit</i></a> 
        <form id="delete-form-{{ $accountforms->id }}" action="{{ route('AccountForm.destroy', $accountforms->id) }}" style="display: none;" method="post">
          @csrf
          @method('delete')
        </form>
        ...
        if(confirm('Are you sure? You want to delete this?')){
           ...
           document.getElementById('delete-form-{{ $accountforms->id }}').submit();
          }else {
           event.preventDefault();
          }
