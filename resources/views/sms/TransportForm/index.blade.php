...
<tbody>
    @foreach($transportform as $key=>$transportforms)
   <tr>
       <td>{{ $key + 1 }}</td>       
       <td>{{ $transportforms->route_name }}</td>
       <td>{{ $transportforms->vehicle_number }}</td>
       <td>{{ $transportforms->driver_name }}</td>
       <td>{{ $transportforms->license_number }}</td>
       <td>{{ $transportforms->phone }}</td>
       <td>
            <a href="{{ route('TransportForm.edit', $transportforms->id) }}" class="btn btn-info btn-sm"><i class="material-icons">mode_edit</i></a> 
        <form id="delete-form-{{ $transportforms->id }}" action="{{ route('TransportForm.destroy', $transportforms->id) }}" style="display: none;" method="post">
          @csrf
          @method('delete')
        </form>
        ...
        if(confirm('Are you sure? You want to delete this?')){
           ...
           document.getElementById('delete-form-{{ $transportforms->id }}').submit();
          }else {
           event.preventDefault();
          }
