...
<tbody>
    ...
    @foreach($parentForm as $key=>$parentForms)
   <tr>
       <td>{{ $key + 1 }}</td>
       <td>{{ $parentForms->idno }}</td>
       <td><img src="{{ asset('uploads/parentimages/'.$parentForms->photo) }}" class="img-circle img-rounded" width="60" alt="no pic"></td>
       <td>{{ $parentForms->fullname }}</td>
       <td>{{ $parentForms->gender }}</td>
       <td>{{ $parentForms->occupation }}</td>
       <td>{{ $parentForms->address }}</td>
       <td>{{ $parentForms->phone }}</td>
       <td>{{ $parentForms->email }}</td>
       <td>
       <a href="{{ route('ParentForm.edit', $parentForms->id) }}" class="btn btn-info btn-sm"><i class="material-icons">mode_edit</i></a> 
        <form id="delete-form-{{ $parentForms->id }}" action="{{ route('ParentForm.destroy', $parentForms->id) }}" style="display: none;" method="post">
          @csrf
          @method('delete')
        </form>
        ...
        if(confirm('Are you sure? You want to delete this?')){
           ...
           document.getElementById('delete-form-{{ $parentForms->id }}').submit();
          }else {
           event.preventDefault();
          }
