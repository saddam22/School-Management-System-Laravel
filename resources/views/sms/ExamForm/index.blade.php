...
<tbody>
    @foreach($examform as $key=>$examforms)
   <tr>
       <td>{{ $key + 1 }}</td>
       <td>{{ $examforms->grade_name }}</td>
       <td>{{ $examforms->grade_point }}</td>
       <td>{{ $examforms->percentage_form }}</td>
       <td>{{ $examforms->percentage_upto }}</td>
       <td>{{ $examforms->comments }}</td>
       <td>
            <a href="{{ route('ExamForm.edit', $examforms->id) }}" class="btn btn-info btn-sm"><i class="material-icons">mode_edit</i></a> 
        <form id="delete-form-{{ $examforms->id }}" action="{{ route('ExamForm.destroy', $examforms->id) }}" style="display: none;" method="post">
          @csrf
          @method('delete')
        </form>
        ...
        if(confirm('Are you sure? You want to delete this?')){
           ...
           document.getElementById('delete-form-{{ $examforms->id }}').submit();
          }else {
           event.preventDefault();
          }
