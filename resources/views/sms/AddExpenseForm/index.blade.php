...
<tbody>
    @foreach($addexpenseform as $key=>$addexpenseforms)
   <tr>
       <td>{{ $key + 1 }}</td>
       <td>{{ $addexpenseforms->idno }}</td>
       <td>{{ $addexpenseforms->photo }}</td>
       <td>{{ $addexpenseforms->name }}</td>
       <td>{{ $addexpenseforms->gender }}</td>
       <td>{{ $addexpenseforms->class }}</td>
       <td>{{ $addexpenseforms->section }}</td>
       <td>{{ $addexpenseforms->expense_type }}</td>
       <td>{{ $addexpenseforms->amount }}</td>
       <td>{{ $addexpenseforms->status }}</td>
       <td>{{ $addexpenseforms->phone }}</td>
       <td>
            <a href="{{ route('AddExpenseForm.edit', $addexpenseforms->id) }}" class="btn btn-info btn-sm"><i class="material-icons">mode_edit</i></a> 
        <form id="delete-form-{{ $addexpenseforms->id }}" action="{{ route('AddExpenseForm.destroy', $addexpenseforms->id) }}" style="display: none;" method="post">
          @csrf
          @method('delete')
        </form>
        ...
        if(confirm('Are you sure? You want to delete this?')){
           ...
           document.getElementById('delete-form-{{ $addexpenseforms->id }}').submit();
          }else {
           event.preventDefault();
          }
